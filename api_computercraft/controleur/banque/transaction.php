                ...
                case "commande":
                    if ($_Joueur_['role'] == 2)
                    {
                        ...
                        if(!empty($listecommande))
                        {
                            ...
                            $datatransaction["statuts"] = $statuts;
                            ...
                            $commande->setStatusCommande($listecommande['id'],$commerce_statuts);
                        }
                        else
                        {
                            // pas de transaction
                ...
                case "retrait":
                    if ($_Joueur_['role'] == 1)
                    {
                        ...
                        if($_compte_debiteur_["compte"] >= $_GET['somme'])
                        {
                            $majDebiteur = new Maj($_compte_debiteur_, $bddConnection);
                            $majDebiteur->setNouvellesDonneesCompte($_compte_debiteur_["compte"]-$_GET['somme']);
                            $statuts = "retrait valider";
                            // modif - ok
                            $printmessage = 1;
                        }
                        else
                        {
                            $statuts = "retrait refuser";
                            // solde insufisant
                            $printmessage = 41;
                        }
                        $transaction = new Transaction($_Joueur_['id'], $bddConnection);
                ...
                case "achatoffre":
                    if ($_Joueur_['role'] == 1)
                    {
                        ...
                        if($_compte_debiteur_["compte"] >= $prix_global AND $_GET['somme'] > 0)
                        {
                            ...
                            $majDebiteur->setNouvellesDonneesNbrOffre($_compte_debiteur_["nbr_offre"]+$_GET['somme']);
                            for ($j=0; $j < $_GET['somme']; $j++) {
                                $boutique->setNouvellesOffre();
                            }
                            $statuts = "achat offre valider";
                            // modif - ok
                            $printmessage = 1;
                        }
                        elseif ($_GET['somme'] <= 0)
                        {
                            ...
                            $statuts = "achat offre refuser limite";
                            // limite offre atteint
                            $printmessage = 42;
                        }
                        else
                        {
                            $statuts = "achat offre refuser";
                            // solde insufisant
                            $printmessage = 41;
                        }
                        $transaction = new Transaction($_Joueur_['id'], $bddConnection);
