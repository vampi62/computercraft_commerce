	...
	if($connection->verifymdp($_GET['mdp']))
	{
		...
		if ($_Joueur_['role'] == 10)
		{
			if(isset($_GET['player']) AND !empty($_GET['player']))
			{
				...
				$reqlogin = $bddConnection->prepare('SELECT id FROM liste_users WHERE pseudo = :pseudo'); // verifie que le joueur a suppr existe
				...
				if(!empty($reqlogin))
				{
					$req = $bddConnection->prepare('DELETE FROM liste_users WHERE id = :id_user');
					$req->execute(array(
						'id_user' => $reqlogin["id"]
					));
					// modif - ok
