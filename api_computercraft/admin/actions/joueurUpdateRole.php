...
if(isset($_GET['pseudo']) AND isset($_GET['mdp']) AND !empty($_GET['pseudo']) AND !empty($_GET['mdp']))
{
	...
	if($connection->verifymdp($_GET['mdp']))
	{
		...
		if ($_Joueur_['role'] == 10)
		{
			if(isset($_GET['player']) AND isset($_GET['newdata']) AND !empty($_GET['player']))
			{
				$_GET['player'] = htmlspecialchars($_GET['player']);
				if ($_GET['pseudo'] == $_GET['player'])
				{
				...
				else
				{
					...
					if(!empty($reqlogin))
					{
						$req = $bddConnection->prepare('UPDATE liste_users SET role = :newdata WHERE id = :id_user');
						$req->execute(array(
							'id_user' => $reqlogin["id"],
