function page_tableau_commande_client()
	...
	local colonne_liste = {{text="vendeur     ",champ=5,type="string"},{text="nom          ",champ=4,type="string"},{text="statut  ",champ=16,type="int"},{text="somme ",champ=10,type="int"}}
	...
	for j=1, #global_filtre_liste[liste] do
		...
		if y <= global_max_y_page and y > global_min_y_page then
			table.insert(global_term_objet_write,{x = 1, y = y, text = global_filtre_liste[liste][j][5], back_color = 32768, text_color = 1})
			...
			table.insert(global_term_objet_select,{xmin = 48, xmax = 50, ymin = y, ymax = y, parametre={action="page",id=53,filtre={id={valeur=global_filtre_liste[liste][j][1],type="egal"}}}, back_color = 128})
