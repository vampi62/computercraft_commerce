function page_plus_info_offre_commerce()
	...
	for j=1, #global_liste[liste] do
		if global_filtre["id"]["valeur"] == global_liste[liste][j]["id"] then
			...
			creation_variable({"prix","nbr_dispo"},{global_liste[liste][j]["prix"],global_liste[liste][j]["nbr_dispo"]})
			...
			table.insert(global_term_objet_select,{xmin = 34, xmax = 46, ymin = 11, ymax = 11, parametre={action="page",id=150,filtre={id_offre={valeur=global_liste[liste][j]["id"],type="egal"}}}, back_color = 128})
			...
			local texte_variable = {
				...
				global_liste[liste][j]["type"],
				global_liste[liste][j]["livraison"],
