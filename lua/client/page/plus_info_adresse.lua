function page_plus_info_adresse()
	...
	for j=1, #global_liste[liste] do
		if global_filtre["id"]["valeur"] == global_liste[liste][j][1] then
			...
			creation_variable({"nom","type_adresse"},{global_liste[liste][j][1],global_liste[liste][j][2]})
			...
			if global_liste[liste][j][2] == "2" then
				table.insert(global_term_objet_write,{x = 33, y = 11, text = "liste offres", back_color = 128, text_color = 1})
				table.insert(global_term_objet_select,{xmin = 34, xmax = 46, ymin = 11, ymax = 11, parametre={action="page",id=130,filtre={proprio={valeur=global_session["pseudo"],type="egal"},adresse={valeur=global_liste[liste][j][1],type="egal"}}}, back_color = 128})
			end
