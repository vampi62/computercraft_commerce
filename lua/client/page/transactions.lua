function page_transactions()
	...
	for j=1, #global_liste_transactions do
		local y = global_scroll + j
		if y <= global_max_y_page and y >= global_min_y_page then
			if j == 1 then
				global_limite_scroll_haut = true
			end
			if j == #global_liste_transactions and y == global_max_y_page then
				global_limite_scroll_bas = true
			end
			table.insert(global_objet_write,{x = 1, y = y, text = "                    |                |        |", back_color = 256, text_color = 1})
			table.insert(global_objet_write,{x = 1, y = y, text = global_liste_transactions[j]["nom"], back_color = 256, text_color = 1})
			table.insert(global_objet_write,{x = 11, y = y, text = global_liste_transactions[j]["statut"], back_color = 256, text_color = 1})
			table.insert(global_objet_write,{x = 28, y = y, text = global_liste_transactions[j]["somme"], back_color = 256, text_color = 1})
			table.insert(global_objet_write,{x = 49, y = y, text = "+", back_color = 256, text_color = 1})
			table.insert(global_objet_select,{xmin = 48, xmax = 50, ymin = y, ymax = y, name = "transaction", value = j, back_color = 256}) -- bouton select
	...
	if global_filtre_true then
		table.insert(global_objet_write,{x = 17, y = 19, text = "filtres actif", back_color = 256, text_color = 1})
	else
		table.insert(global_objet_write,{x = 17, y = 19, text = "filtres inactif", back_color = 256, text_color = 1})
	end
	table.insert(global_objet_select,{xmin = 17, xmax = 31, ymin = 19, ymax = 19, name = "page_filtres", back_color = 256})
