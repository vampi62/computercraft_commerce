function page_tableau_commande_commerce()
	...
	local colonne_liste = {{text="acheteur    ",champ="recepteur",type="string"},{text="nom          ",champ="nom_commande",type="string"},{text="statut  ",champ="statut",type="int"},{text="somme ",champ="somme",type="int"}}
	...
	for j=1, #global_filtre_liste[liste] do
		...
		if y <= global_max_y_page and y > global_min_y_page then
			table.insert(global_term_objet_write,{x = 1, y = y, text = global_filtre_liste[liste][j]["recepteur"], back_color = 32768, text_color = 1})
			...
			table.insert(global_term_objet_select,{xmin = 48, xmax = 50, ymin = y, ymax = y, parametre={action="page",id=153,filtre={id={valeur=global_filtre_liste[liste][j]["id"],type="egal"}}}, back_color = 128})
