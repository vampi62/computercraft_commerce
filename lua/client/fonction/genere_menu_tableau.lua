function genere_menu_tableau(colonne_liste,liste)
	...
	for j=1, #colonne_liste do
		...
		table.insert(global_term_objet_select,{xmin = x_colonne, xmax = x_colonne + string.len(colonne_liste[j]["text"].." "), ymin = global_min_y_page, ymax = global_min_y_page, parametre={action="action",id="rangeliste",valeur={liste=liste,champ=colonne_liste[j]["champ"],type=colonne_liste[j]["type"]}}, back_color = 128})
