...
function encode_with_pub(msg)-- Maximum message size is bits / byteSize
    publicKey, privateKey = load_key()
    local byteSize = 8
    local bits = 256
    local startTime = os.clock()
    -- Encrypting
    local res = bytesToNumber(stringToBytes(msg), bits, byteSize)
    return crypt(publicKey, res)
end
function encode_with_priv(msg)-- Maximum message size is bits / byteSize
    publicKey, privateKey = load_key()
    local byteSize = 8
    local bits = 256
    local startTime = os.clock()
    -- Encrypting
    local res = bytesToNumber(stringToBytes(msg), bits, byteSize)
    return crypt(privateKey, res)
end

function decode_with_pub(msg)
    publicKey, privateKey = load_key()
    local byteSize = 8
    local bits = 256
    startTime = os.clock()
    -- Decrypting
    local decrypted = crypt(publicKey, msg)
    local decryptedBytes = numberToBytes(decrypted, bits, byteSize)
    return bytesToString(decryptedBytes)
end
function decode_with_priv(msg)
    publicKey, privateKey = load_key()
    local byteSize = 8
    local bits = 256
    startTime = os.clock()
    -- Decrypting
    local decrypted = crypt(privateKey, msg)
    local decryptedBytes = numberToBytes(decrypted, bits, byteSize)
    return bytesToString(decryptedBytes)
end